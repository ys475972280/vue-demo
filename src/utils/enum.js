...
export default class EnumFactory {
  ...
  constructor(entries, parent) {
    if (
      ...
      throw new TypeError(
        '[EnumFactory]: entries must be Array (2D) and NOT empty'
      )
    }
    ...
    // freeze instance
    if (new.target === EnumFactory) deepFreeze(this)
  }
  ...
  options(unshiftIt, pushIt) {
    let options = []
    if (unshiftIt === true) {
      options.push({label: '全部', value: null})
...
function getKey(extra) {
  const {$key, ...props} = isPrettyObject(extra)
    ? extra
    : extra
      ? {$key: extra}
      : {}
...
function formatList(datas) {
  let options = []
  datas.forEach(([value, label, {...props} = {}]) => {
